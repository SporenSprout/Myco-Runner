...
const LaneGuides: React.FC = () => {
    ...
    return (
        <group position={[0, 0, 0]}>
            {/* Lane Floor - Pushed down to -0.2 to avoid any Z-fighting */}
            <mesh position={[0, -0.2, 0]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={1}>
                <planeGeometry args={[laneCount * LANE_WIDTH, 2000]} />
                <meshBasicMaterial color="#1a0b2e" transparent opacity={0.9} depthWrite={false} />
            </mesh>

            {/* Lane Separators - Raised to 0.1 and renderOrder forced to top */}
            {separators.map((x, i) => (
                <mesh key={`sep-${i}`} position={[x, 0.1, 0]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={10}>
                    {/* Thicker line (0.12) for robust visibility on all screens */}
                    <planeGeometry args={[0.12, 2000]} /> 
                    <meshBasicMaterial 
                        color="#00ffff" 
                        transparent 
                        opacity={0.8} // Increased opacity for solid look
                        depthWrite={false}
                        polygonOffset
                        polygonOffsetFactor={-10} // Strong bias towards camera
                    />
                </mesh>
            ))}
        </group>
...
const MovingGrid: React.FC = () => {
    ...
    return (
        // Shifted X slightly (0.1) to prevent perfect overlap with center lane line
        <mesh ref={meshRef} rotation={[-Math.PI / 2, 0, 0]} position={[0.1, -0.5, -100]} renderOrder={0}>
